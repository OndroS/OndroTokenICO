const OndroTokenIco = artifacts.require('./contracts/OndroToken.sol');
...
contract('OndroTokenIco', accounts => {
  ...
  const createToken = () => OndroTokenIco.new(fundsWallet, timeController.currentTimestamp(), minCap, maxCap);

  // REQ001: Basic ERC20 “Ondro Token” with symbol of “DRB”,
  ...
  it('should have initial supply of 1,000,000 units assigned to funds wallet', async () => {
    const ondroToken = await createToken();
    const expectedSupply = toWei(1000000);

    const totalSupply = await ondroToken.totalSupply();
    assert.equal(totalSupply, expectedSupply, 'Total supply mismatch');

    const fundsWalletBalance = await ondroToken.balanceOf(fundsWallet);
  ...
  it('should open at startTimestamp', async () => {
    ...
    const ondroToken = await OndroTokenIco.new(fundsWallet, startTimestamp, minCap, maxCap);

    // should be closed
    await assertExpectedError(ondroToken.sendTransaction(transaction(buyerOneWallet, oneEth)));

    await timeController.addSeconds(3600);

    // should be open
    await ondroToken.sendTransaction(transaction(buyerOneWallet, oneEth));
  });

  it('should last 4 weeks if the goal is reached and allow token transfers afterwards', async () => {
    const ondroToken = await createToken();

    await ondroToken.sendTransaction(transaction(buyerOneWallet, minCap));
    await timeController.addDays(4 * 7);

    // should be closed
    await assertExpectedError(ondroToken.sendTransaction(transaction(buyerTwoWallet, oneEth)));

    const totalRaised = await ondroToken.totalRaised();
    assert.equal(totalRaised.toNumber(), minCap, 'Total raised amount mismatch')

    // REQ004 should allow token transfer
    await ondroToken.transfer(buyerTwoWallet, 1, { from: buyerOneWallet });
  });
  ...
  it('should last more then 4 weeks if the goal is not reached and allow token transfers after closing', async () => {
    ...
    await ondroToken.sendTransaction(transaction(buyerTwoWallet, minCap - oneEth));

    // should be closed
    await assertExpectedError(ondroToken.sendTransaction(transaction(buyerThreeWallet, oneEth)));
  ...
  it('should close after the max cap is reached and allow token transfers afterwards', async () => {
    ...
    await ondroToken.sendTransaction(transaction(buyerTwoWallet, maxCap));
  ...
  it('should transfer raised funds to fundsWallet after each payment', async () => {
    ...
    assert.equal(ethBalance(ondroToken.address), 0, 'Contract balance mismatch');
    assert.equal(ethBalance(fundsWallet), expectedBalanceGrowth(oneEth), 'Funds wallet balance mismatch');

    await ondroToken.sendTransaction(transaction(buyerTwoWallet, oneEth));
  ...
  it('should not allow token transfers before ICO', async () => {
    ...
    await assertExpectedError(ondroToken.transfer(buyerTwoWallet, 1, { from: buyerOneWallet }));
  });
  ...
  // REQ005: The tokens are going to be sold at a flat rate of 1 ETH : 50 DRB,
  ...
  it('should be sold with +50% bonus during the first week', async () => {
    ...
    const buyerOneBalance = await ondroToken.balanceOf(buyerOneWallet);
